...
void dial_user(tbl* teleRecord, tbl* billInfo,  int IDu)
{
	...
	if(billInfo->clm.phint[IDu][9] > 0)
	{
		...
		else
		{
			...
			time[0]=startTime;
			time[1]=endTime;	
			cpyrow(teleRecord,inte,nam,time);
	...
	while(row_locater<=row)
	{
		if(User->clm.phint[row_locater][4] == IDa)
		{
			...
			while(rowMoney_locater<=rowMoney)
			{
				...
				{
					printf("\ntime:");
					display_tim(moneyRecord->clm.phtim[rowMoney_locater][0]);
				}
				rowMoney_locater++;
			}
			counter_pass++;
	...
	while(row_locater<=row)
	{
		if(sets->clm.phint[row_locater][2]==provider)
		{
			...
			printf("netAmount:%d\n",sets->clm.phint[provider][4]);
 			printf("bortime:");
			display_tim(sets->clm.phtim[provider][0]);
			printf("\n");
 			printf("fortime:");
			display_tim(sets->clm.phtim[provider][1]);
			printf("\n");
